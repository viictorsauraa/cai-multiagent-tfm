...
def get_available_agents() -> Dict[str, Agent]:  # pylint: disable=R0912  # noqa
    ...
    # ONLY for patterns that don't already have a real Agent in agents_to_display.
    # Some patterns (e.g. redteam_orchestrated_pattern) export a real Agent with
    # handoffs, model, and _lock_model.  Overwriting them with a bare PatternAgent
    # (handoffs=[], model=None) would break the swarm.
    from cai.agents.patterns import PATTERNS
    for pattern_name, pattern_obj in PATTERNS.items():
        if pattern_name in agents_to_display and isinstance(agents_to_display[pattern_name], Agent):
            # Real agent already registered — skip pseudo-agent
            continue
    ...
    if hasattr(agent, "model") and hasattr(agent.model, "__class__"):
        try:
            ...
            if model_class.__name__ == "OpenAIChatCompletionsModel":
                ...
                # Propagate _lock_model (dynamic attr, not a dataclass field)
                if getattr(agent, "_lock_model", False):
                    cloned_agent._lock_model = True
