...
def create_generic_agent_factory(
    ...
    def factory(model_override: str | None = None, custom_name: str | None = None, agent_id: str | None = None):
        ...
        # If the original agent has _lock_model, respect its model assignment
        # instead of using CAI_MODEL. This preserves multi-model swarm patterns.
        if getattr(original_agent, "_lock_model", False) and not model_override:
            new_model = original_agent.model
        ...
        if getattr(original_agent, "_lock_model", False):
