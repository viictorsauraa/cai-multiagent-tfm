...
Tool call examples (ALWAYS use function calling, never write JSON as text):
  ...
  generic_linux_command(command="curl -s 'http://target.com/api/v1/users' -H 'Authorization: Bearer TOKEN'")

  generic_linux_command(command="ffuf -u http://target.com/FUZZ -w /usr/share/wordlists/dirb/common.txt -mc 200,301,302,403")

  generic_linux_command(command="gau target.com | sort -u | head -100")

  generic_linux_command(command="waybackurls target.com | grep '=' | head -50")

  execute_code(code="import requests\n# Test for IDOR\nfor uid in range(1,10):\n    r = requests.get(f'http://target.com/api/user/{uid}')\n    print(f'User {uid}: {r.status_code}')", language="python")

  transfer_to_dns_agent()
