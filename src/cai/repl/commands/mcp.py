...
"""
    ...
    def handle_add(self, args: Optional[List[str]] = None) -> bool:
        ...
        try:
            ...
            # We MUST mutate the list in-place (slice assignment) — if we
            # reassigned `agent.tools = [...]`, we would create a new list
            # object, which would break the shared-reference invariant that
            # patterns like redteam_orchestrated rely on (clones share the
            # tools list via shallow-copy dataclasses.replace, so in-place
            # mutation propagates but reassignment does not).
