...
"""
    ...
    async def _ai_summarize_history(
        self,
        ...
        Args:
            agent_name: Name of the agent whose history to summarize.
            history_override: If provided, use this list as the history instead
                of looking it up via AGENT_MANAGER (needed for isolated_context
                agents whose model.message_history diverges from the manager).
            prompt_override: If provided, use this as the summarization prompt
                instead of the default or custom compact prompt.
        """
        # Get history to summarize
        if history_override is not None:
            history = history_override
            target = f"agent '{agent_name}'" if agent_name else "provided history"
        elif agent_name:
            history = get_agent_message_history(agent_name)
        ...
        # Use prompt_override (auto-compact briefing), custom prompt, or default
...
This session is being continued from a previous conversation that ran out of context. The conversation is summarized below:"""
        ...
        # Build client matching the active backend (Ollama or OpenAI)
        ollama_base = os.getenv("OLLAMA_API_BASE")
        if ollama_base:
            base_url = ollama_base if ollama_base.endswith("/v1") else f"{ollama_base}/v1"
            client = AsyncOpenAI(
                api_key=os.getenv("OLLAMA_API_KEY") or os.getenv("OPENAI_API_KEY", "ollama"),
                base_url=base_url,
            )
        else:
        ...
        # Reset the Summary Agent's persisted message history before this run.
        # OpenAIChatCompletionsModel.__init__ inherits message_history from
        # AGENT_MANAGER by agent_name. Without this clear, every successive
        # auto-compact run on the same process accumulates the previous
        # "summarize: ..." / "<summary>" pairs from prior compactions,
        # ballooning the next summarization input and causing the compactor
        # to keep retriggering with ever-larger payloads.
        try:
            AGENT_MANAGER.clear_history("Summary Agent")
        except Exception:
            pass
