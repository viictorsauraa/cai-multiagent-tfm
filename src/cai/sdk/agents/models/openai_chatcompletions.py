...
# Global registry: tool call_id → agent_name that originated the call.
# Ensures tool outputs are displayed with the correct agent name even when
# items_to_messages is called on a DIFFERENT agent's model (after handoff).
_TOOL_CALL_AGENT_REGISTRY: dict[str, str] = {}

# Cache for Ollama model context lengths (queried via /api/show).
# Avoids repeated HTTP requests during a session.
_ollama_context_cache: dict[str, int] = {}

# Fallback context lengths when model info is unavailable.
# CLOUD_DEFAULT: cloud APIs (OpenAI, Anthropic) typically support large contexts.
# OLLAMA_FALLBACK: conservative default for local Ollama models when /api/show
#   is unreachable — most Ollama models default to 2048–8192, but many are
#   configured with num_ctx up to 32768.
_CLOUD_DEFAULT_MAX_TOKENS = 200_000
_OLLAMA_FALLBACK_MAX_TOKENS = int(os.getenv("CAI_OLLAMA_FALLBACK_CTX", "32768"))
...
def _token_estimate_multiplier() -> float:
    """Calibration multiplier applied to tiktoken estimates.

    tiktoken's cl100k_base encoding consistently UNDER-counts tokens for
    non-English-trained models served via Ollama (qwen, llama, etc.) by
    roughly 30-50%. Measured ratio (real prompt_tokens reported by Ollama
    / tiktoken estimate) on a JWT pentest run with qwen2.5:32b: avg=1.41,
    median=1.43, max=1.78.

    Without correction:
    - The CAI_CONTEXT_USAGE toolbar value (used by the auto-compact
      guardrail and the UI percentage) reads ~40% lower than reality.
    - The 80% auto-compact threshold therefore fires at what is
      effectively ~110% of the model's real context window — i.e. never,
      because Ollama silently truncates first.

    Override with env var ``CAI_TOKEN_ESTIMATE_MULTIPLIER``. Default
    1.4 when ``OLLAMA_API_BASE`` is set (Ollama backend), 1.0 otherwise
    (cloud models served via OpenAI-compatible APIs that report tokens
    accurately and tiktoken estimates with low error).
    """
    override = os.getenv("CAI_TOKEN_ESTIMATE_MULTIPLIER")
    if override is not None:
        try:
            return float(override)
        except ValueError:
            pass
    return 1.4 if os.getenv("OLLAMA_API_BASE") else 1.0
...
class OpenAIChatCompletionsModel(Model):
    ...
    def __init__(
        ...
        _ollama_env = os.getenv("OLLAMA", "").lower()
        self.is_ollama = (
            (_ollama_env != "" and _ollama_env != "false")
            or os.getenv("OLLAMA_API_BASE") is not None
        )
    ...
    async def get_response(
        ...
        with generation_span(
            ...
            # The tiktoken estimate is then scaled by the backend-specific
            # multiplier (1.4 for Ollama, 1.0 for cloud) — see
            # _token_estimate_multiplier() for the rationale.
            raw_estimate, _ = count_tokens_with_tiktoken(converted_messages)
            estimated_input_tokens = int(raw_estimate * _token_estimate_multiplier())

            # Calculate and set context usage for toolbar (uses the calibrated
            # estimate so the percentage shown matches the real prompt_tokens
            # reported by the server, which is also what the auto-compact
            ...
            # --- Retry-with-nudge guardrail for specialist agents ---
            # Two failure modes can cause a specialist (can_finish=False) to
            # stall the engagement without invoking any tool:
            #
            # (a) Empty response with tokens spent on <think> blocks (qwen3,
            #     deepseek-r1, …): completion_tokens > 50 but both content
            #     and tool_calls are empty.
            # (b) Narrative drift: the model returns prose describing what
            #     it would do but never emits a tool_call. Observed in
            #     qwen2.5:32b mid-engagement when stuck on a failing vector.
            #     Without intervention, the orchestrator absorbs the text
            #     and ends the engagement via final_output, even though the
            #     specialist never actually acted.
            #
            # In both cases, retry once with a nudge that demands a tool call.
            # The guardrail only fires for specialists — agents with
            # can_finish=True are allowed to terminate with a text response.
            _msg = response.choices[0].message
            _empty_content = not getattr(_msg, "content", None) or not str(_msg.content).strip()
            _empty_tools = not getattr(_msg, "tool_calls", None)
            _has_tokens = (
                response.usage
                and getattr(response.usage, "completion_tokens", 0) > 50
            )
            _specialist = not getattr(self, '_agent_can_finish', True)

            # Trigger when specialist returned without any tool_call AND
            # either:
            #   - drift: produced prose but no tool_call (case b), OR
            #   - blank: produced nothing despite spending tokens (case a).
            # An empty response with very few tokens is treated as a
            # transient API hiccup and NOT retried (avoids loops on
            # genuine connectivity issues).
            _drift = not _empty_content
            _blank = _empty_content and _has_tokens
            _should_nudge = _specialist and _empty_tools and (_drift or _blank)
            if _should_nudge:
                _drift = not _empty_content
                logger.warning(
                    "%s — retrying with nudge",
                    "Narrative drift (text without tool_call)" if _drift
                    else f"Empty response with {response.usage.completion_tokens} completion tokens",
                )
                # Add nudge to message_history so _fetch_response sees it
                nudge_msg = {
                    "role": "user",
                    "content": (
                        "Your previous response did not invoke any tool. "
                        "You MUST respond with a tool call. As a specialist "
                        "agent you cannot end the engagement with prose — "
                        "either execute the next action (generic_linux_command, "
                        "native helpers, MCP tools, write_key_findings/"
                        "read_key_findings) or hand off "
                        "(transfer_to_red_team_orchestrator) if you genuinely "
                        "cannot make progress. Do NOT narrate plans without acting."
                    ),
                }
                self.add_to_message_history(nudge_msg)
                try:
                    response = await self._fetch_response(
                        system_instructions,
                        input,
                    ...
                    logger.debug("Nudge retry failed, keeping original response")
                finally:
                    # Remove the nudge from history to avoid contaminating future turns
                    if self.message_history and self.message_history[-1] is nudge_msg:
            ...
            if hasattr(assistant_msg, "tool_calls") and assistant_msg.tool_calls:
                ...
                for tool_call in assistant_msg.tool_calls:
                    ...
                    # Register the originating agent for correct display after handoffs
                    _TOOL_CALL_AGENT_REGISTRY[tool_call.id] = self.agent_name
            ...
            # Include both function tools and handoff tools for debugging
            logged_tools = [t.params_json_schema for t in tools] if tools else []
            for ho in handoffs:
                logged_tools.append({"name": ho.tool_name, "type": "handoff", "agent": ho.agent_name})
                    ...
                    for item in input:
                        if isinstance(item, dict):
                            if item.get("role") == "user":
                                ...
                                if item.get("content"):
                                    self.logger.log_user_message(item.get("content"))
                # Get token count estimate before API call (calibrated for the backend).
                raw_estimate, _ = count_tokens_with_tiktoken(converted_messages)
                estimated_input_tokens = int(raw_estimate * _token_estimate_multiplier())
    ...
    async def _fetch_response(
        ...
        if self.message_history:
            # Swarm / multi-turn mode: message_history already contains the
            # full conversation.  generated_items (via input) duplicates it,
            # so we use ONLY message_history to avoid sending every message
            # twice (which wastes tokens and confuses the model).
            for msg in self.message_history:
                msg_copy = msg.copy()
                if "cache_control" in msg_copy:
                    del msg_copy["cache_control"]
            ...
            # First turn or no history: convert input items to messages
            new_messages = self._converter.items_to_messages(input, model_instance=self)
            converted_messages.extend(new_messages)

        # Ensure no orphan tool_calls without matching tool responses
        # (can happen after swarm handoffs when history is shared)
        converted_messages = self._sanitize_tool_call_pairs(converted_messages)
        ...
        # Add retry logic for rate limits and server busy errors
        max_retries = 5
        retry_count = 0
        ...
        while retry_count < max_retries:
            try:
                ...
                if retry_count >= max_retries:
                    print(
                        f"\n❌ Model '{model_str}' timed out after {max_retries} attempts.\n"
                        f"   Agent: {self.agent_name}\n"
                        f"   Check Ollama server status or increase CAI_MODEL_TIMEOUT (current: {os.getenv('CAI_MODEL_TIMEOUT', '300')}s)"
                    )
                    raise
                print(f"\n⏳ Model '{model_str}' timed out (attempt {retry_count}/{max_retries}), retrying...")
                continue

            except litellm.exceptions.ServiceUnavailableError as e:
                retry_count += 1
                if retry_count >= max_retries:
                    print(f"\n❌ Server unavailable after {max_retries} retries")
                    raise
                import random
                retry_delay = min(60, 5 * retry_count) + random.randint(0, 5)
                print(f"\n⏳ Server busy (503) - attempt {retry_count}/{max_retries}, retrying in {retry_delay}s...")
                await asyncio.sleep(retry_delay)
                continue

            except litellm.exceptions.NotFoundError as e:
                # Model not found in Ollama (404) — no retry, clear message
                error_msg = str(e)
                model_str = str(self.model)
                print(
                    f"\n❌ Model '{model_str}' not found on the server.\n"
                    f"   Pull it with:  ollama pull {model_str}\n"
                    f"   Agent: {self.agent_name}"
                )
                raise
                ...
                # Handle model-not-found errors that come as 400 instead of 404
                if "model" in error_msg.lower() and (
                    "not found" in error_msg.lower()
                    or "does not exist" in error_msg.lower()
                    or "pull the model" in error_msg.lower()
                ):
                    model_str = str(self.model)
                    print(
                        f"\n❌ Model '{model_str}' not available.\n"
                        f"   Pull it with:  ollama pull {model_str}\n"
                        f"   Agent: {self.agent_name}"
                    )
                    raise
                ...
                if (
                    ...
                    if match1:
                        ...
                        try:
                            est_tokens, _ = count_tokens_with_tiktoken(kwargs.get("messages", []))
    ...
    async def _fetch_response_litellm_ollama(
        ...
        if stream:
            ...
            _timeout = int(os.getenv("CAI_MODEL_TIMEOUT", "300"))
    ...
    @staticmethod
    def _sanitize_tool_call_pairs(messages: list[dict]) -> list[dict]:
        """Ensure every assistant message with tool_calls has matching tool responses.

        After a swarm handoff the shared message_history may contain the
        previous agent's tool_calls without the corresponding tool-role
        responses (because they were appended to generated_items, not to
        message_history).  Ollama / OpenAI reject such sequences silently
        or with a cryptic error, causing the new agent to hang.

        This method scans for orphan tool_calls and adds placeholder tool
        responses so the message sequence is always valid.
        """
        ...
        # Find orphan tool_calls and insert placeholders
        result: list[dict] = []
        for msg in messages:
            result.append(msg)
            if msg.get("role") == "assistant" and msg.get("tool_calls"):
                for tc in msg["tool_calls"]:
                    ...
                    if tc_id and tc_id not in tool_response_ids:
                        ...
                        result.append({
                            "role": "tool",
                            "tool_call_id": tc_id,
                            "content": f"[handoff — {fn_name} processed by previous agent]",
                        })
    ...
    def _get_model_max_tokens(self, model_name: str) -> int:
        ...
        Resolution order:
        1. pricing.json (cloud models)
        2. _ollama_context_cache (previously queried Ollama models)
        3. Ollama /api/show endpoint (live query, result is cached)
        4. Conservative fallback: 32768 if OLLAMA_API_BASE is set, else 200000
        """
        # 1. Check pricing.json first (cloud models)
        try:
            import pathlib
            ...
            if pricing_path.exists():
                with open(pricing_path, encoding="utf-8") as f:
                    ...
                    if model_name in pricing_data:
                        ...
                        return model_info.get("max_input_tokens", _CLOUD_DEFAULT_MAX_TOKENS)
        except Exception:
            pass

        # 2. Check Ollama context length cache
        if model_name in _ollama_context_cache:
            return _ollama_context_cache[model_name]

        # 3. Query Ollama API for actual context length
        try:
            ...
            for key, val in model_info.items():
                if "context_length" in key:
                    ctx = int(val)
                    _ollama_context_cache[model_name] = ctx
                    return ctx
        except Exception:
            pass

        # 4. Fallback: conservative for Ollama, large for cloud
        if os.getenv("OLLAMA_API_BASE"):
            return _OLLAMA_FALLBACK_MAX_TOKENS
        return _CLOUD_DEFAULT_MAX_TOKENS

    # Structured briefing prompt for auto-compact summaries.
    # Produces output compatible with handoff briefing format so agents
    # can consume compacted context the same way they consume briefings.
    AUTO_COMPACT_BRIEFING_PROMPT = """You are a context compressor for a multi-agent security testing system.
Summarize the conversation into a STRUCTURED BRIEFING with these exact sections.
Only include sections that have relevant content.

## Objective
The original goal/target (IP, URL, or task description).

## User Instructions (HIGH PRIORITY)
Any explicit instructions from the user (NOT from other agents).

## Commands Executed
List of commands/tools run (last 15 max), one per line with "- " prefix.

## Key Findings
Important results: open ports, vulnerabilities found, credentials, flags, access gained.

## Current Status
Where the work stands: what was achieved, what failed, what's in progress.

## Next Steps
What should be done next based on findings so far.

Keep it concise. Use bullet points. Preserve exact IPs, ports, paths, credentials, and flags.
Do NOT include generic advice — only information specific to this engagement."""
    ...
    async def _auto_compact_if_needed(self, estimated_tokens: int, input: str | list[TResponseInputItem], system_instructions: str | None) -> tuple[str | list[TResponseInputItem], str | None, bool]:
        ...
        try:
            ...
            # Generate AI summary using the model's actual history (not AGENT_MANAGER's
            # potentially stale reference — critical for isolated_context agents)
            summary = await MEMORY_COMMAND_INSTANCE._ai_summarize_history(
                self.agent_name,
                history_override=list(self.message_history),
                prompt_override=self.AUTO_COMPACT_BRIEFING_PROMPT,
            )

            if summary:
                # Store the summary as a list (consistent with /compact manual)
                # and set APPLIED_MEMORY_IDS to prevent .append() crash
                ...
                APPLIED_MEMORY_IDS[self.agent_name] = [f"auto_{int(_time.time())}"]
...
class _Converter:
    ...
    # CAI-internal bookkeeping markers that may piggy-back on
    # user/assistant messages without changing their semantics. They are
    # filtered out of the key-set check so the message is recognized as
    # a plain EasyInputMessageParam — they are NOT read again downstream
    # inside items_to_messages (only the role/content fields are used).
    _EASY_INPUT_INTERNAL_KEYS = {"_hitl_logged"}

    def maybe_easy_input_message(self, item: Any) -> EasyInputMessageParam | None:
        if not isinstance(item, dict):
            return None

        # EasyInputMessageParam only has {content, role}. Allow whitelisted
        # internal markers (see _EASY_INPUT_INTERNAL_KEYS) to coexist.
        public_keys = set(item.keys()) - self._EASY_INPUT_INTERNAL_KEYS
        if public_keys != {"content", "role"}:
            return None
        ...
        for item in items:
            ...
            # 4) function/file-search calls => attach to assistant
                ...
                self.recent_tool_calls[func_call["call_id"]] = {
                    ...
                    "start_time": time.time(),
                    "execution_info": {"start_time": time.time()},
                    # Use the global registry to get the correct originating agent name
                    "agent_name": _TOOL_CALL_AGENT_REGISTRY.get(
                        func_call["call_id"],
                        getattr(model_instance, "agent_name", "Agent") if model_instance else "Agent"
                    ),
                }
            ...
            # 5) function call output => tool message
                ...
                token_info = {
                    ...
                    # Use the originating agent name from tool call metadata
                    # (correct after handoffs) instead of model_instance.agent_name
                    "agent_name": (
                        self.recent_tool_calls[call_id].get("agent_name")
                        if hasattr(self, "recent_tool_calls") and call_id in self.recent_tool_calls
                        else None
                    ) or _TOOL_CALL_AGENT_REGISTRY.get(
                        call_id, getattr(model_instance, "agent_name", "Agent")
                    ),
                }
            ...
            # Log the offending item shape before raising so the next
            # occurrence is diagnosable instead of opaque. Common cause:
            # a chat-completion-format message ({role:"tool",...} or
            # {role:"assistant", tool_calls:[...]}) slipped into history
            # after a Ctrl+C or a narrative-drift retry.
            else:
                try:
                    preview = {
                    ...
                    preview = {"repr": repr(item)[:200]}
                logger.warning(
                    "items_to_messages: invalid item shape %r — raising UserError",
                    preview,
                )
