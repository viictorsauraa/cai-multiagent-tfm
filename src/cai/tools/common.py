...
def _kill_process_tree(process):
    ...
    Falls back to ``process.kill()`` if the group kill fails.
    """
    try:
...
async def _run_local_async(command, stdout=False, timeout=100, stream=False, call_id=None, tool_name=None, workspace_dir=None, custom_args=None, idle_timeout=10, max_execution_time=0):
    ...
    try:
        ...
        if stream:
            ...
            # Start the async process in its own session so we can kill
            # the entire process group (tool + child processes) on timeout.
            ...
            while True:
                if process.returncode is not None:
                    break
                if max_execution_time > 0 and time.time() - process_start_time > max_execution_time:
                    _kill_process_tree(process)
                    await process.wait()
                    output_buffer.append(f"\n[Terminated: max execution time {max_execution_time}s reached]")
                    break
                try:
            ...
            while True:
                ...
                if max_execution_time > 0 and time.time() - process_start_time > max_execution_time:
                    _kill_process_tree(process)
                    await process.wait()
                    stderr_chunks.append(f"\n[Terminated: max execution time {max_execution_time}s reached]".encode())
                    break
                if process.returncode is not None:
                    break
                try:
...
async def _run_docker_async(command, container_id, stdout=False, timeout=100, stream=False, call_id=None, tool_name=None, args=None, idle_timeout=10, max_execution_time=0):
    ...
    try:
        ...
        if stream:
            ...
            while True:
                if process.returncode is not None:
                    break
                if max_execution_time > 0 and time.time() - start_time > max_execution_time:
                    process.terminate()
                    try:
