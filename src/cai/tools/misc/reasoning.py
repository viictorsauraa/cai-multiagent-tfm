...
def _state_file_path() -> str:
    """Return absolute path to state.txt, anchored to workspace if available."""
    try:
        from cai.tools.common import _get_workspace_dir
        workspace = _get_workspace_dir()
    except ImportError:
        workspace = (
            os.environ.get("CAI_WORKSPACE_DIR")
            or os.getcwd()
        )
    return os.path.join(workspace, "state.txt")
...
def write_key_findings(findings: str) -> str:
    ...
    try:
        ...
        try:
            ...
            pass
        # Only append if findings not already present
        if findings.strip() in existing:
            return f"Findings already recorded in state.txt (skipped duplicate):\n{findings}"
