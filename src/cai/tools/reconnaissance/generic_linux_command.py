...
async def generic_linux_command(command: str = "",
    ...
    # Apply tool-profile smart defaults (modular, auto-discovered).
    # Each tool's enhancements live in tool_profiles/<tool>.py.
    # The model chooses command flags; profiles only intervene when
    # without them the tool would fail (e.g. sqlmap CRITICAL error).
    from cai.tools.reconnaissance.tool_profiles import apply_profiles_pre, get_idle_timeout, get_max_execution_time
    command = apply_profiles_pre(command)
    idle_timeout = get_idle_timeout(command)
    max_execution_time = get_max_execution_time(command)

    # Ensure timeout >= max_execution_time so the profile's limit takes
    # effect.  Without this, the default timeout=100 fires before
    # max_execution_time=300 (e.g. sqlmap) in the non-streaming path.
    if max_execution_time > 0 and max_execution_time > timeout:
    ...
    # Apply tool-profile post-processing (e.g. output parsing/summarization)
    from cai.tools.reconnaissance.tool_profiles import apply_profiles_post
    if isinstance(result, str):
        result = apply_profiles_post(command, result)
