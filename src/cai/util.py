...
def fix_message_list(messages):  # pylint: disable=R0914,R0915,R0912
    ...
    relocated_tool_ids: set = set()  # Guard against infinite swap loops
    i = 0
    while i < len(processed_messages):
        ...
        if msg.get("role") == "tool" and msg.get("tool_call_id"):
            ...
            # If this tool message was already relocated once, skip it to
            # prevent infinite pop/insert cycles (e.g. two sibling tool
            # responses for the same assistant swapping each other forever).
            if tool_id in relocated_tool_ids:
                i += 1
                continue
            ...
            if i > 0:
                ...
                if not is_valid_sequence:
                    ...
                    if assistant_idx is not None:
                        ...
                        relocated_tool_ids.add(tool_id)
...
def get_model_input_tokens(model):
    ...
    Delegates to OpenAIChatCompletionsModel._get_model_max_tokens() which
    queries Ollama /api/show and caches the result.  Falls back to a
    hardcoded table only when that import is unavailable.
    """
    try:
        from cai.sdk.agents.models.openai_chatcompletions import (
            OpenAIChatCompletionsModel,
        )
        # Use a lightweight instance just to call the resolution method
        # (it only needs self.model, which we don't use here).
        return OpenAIChatCompletionsModel._get_model_max_tokens(None, str(model))
    except Exception:
        pass

    # Fallback: hardcoded table (only reached if import fails)
    model_tokens = {
        "gpt": 128000,
        "o1": 200000,
        "claude": 200000,
...
def _create_token_display(
    ...
    # Context usage — keep last known percentage for turns without model calls
    tokens_text.append(" | ", style="dim")
    if interaction_input_tokens > 0:
        max_ctx = get_model_input_tokens(model_name)
        context_pct = interaction_input_tokens / max_ctx * 100 if max_ctx > 0 else 0.0
        _create_token_display._last_context_pct = context_pct
    else:
        context_pct = getattr(_create_token_display, '_last_context_pct', 0.0)
...
def _print_simple_tool_output(tool_name, args, output, execution_info=None, token_info=None):
    ...
    if token_info:
        ...
        if interaction_input_tokens > 0 or total_input_tokens > 0:
            ...
            # Show context usage — keep last known percentage for tool-output turns
            if interaction_input_tokens > 0:
                max_ctx = get_model_input_tokens(model)
                context_pct = interaction_input_tokens / max_ctx * 100 if max_ctx > 0 else 0.0
                _create_token_display._last_context_pct = context_pct
            else:
                context_pct = getattr(_create_token_display, '_last_context_pct', 0.0)
